    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`group relative w-16 h-16 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full shadow-2xl hover:shadow-blue-500/25 transition-all duration-300 flex items-center justify-center ${
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[600px] bg-white/95 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/20 z-40 flex flex-col overflow-hidden animate-scaleIn">
          {/* Header */}
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <div>
                  <h3 className="text-xl font-bold">SmartTech AI</h3>
                  <div className="flex items-center space-x-2">
                    <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                    <span className="text-white/80 text-sm">Online Assistant</span>
                  </div>
                </div>
              </div>
              <p className="text-white/80 text-sm">Ask me anything about our Interactive Smart Board!</p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-slate-50 to-blue-50">
            {chatMessages.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-slate-600 font-medium mb-2">👋 Hi! I&apos;m your AI assistant.</p>
                <p className="text-slate-500 text-sm">Ready to help with your Smart Board questions!</p>
                
                {/* Quick Action Buttons */}
                <div className="mt-6 space-y-2">
                  <button
                    onClick={() => setChatInput('What are the key features of the Smart Board?')}
                    className="w-full text-left p-3 bg-white/80 hover:bg-white rounded-xl border border-white/50 hover:shadow-md transition-all duration-300 text-sm text-slate-700"
                  >
              ...
              <div key={index} className={`flex ${msg.isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] ${
                  msg.isUser 
                    ...
                    : 'bg-white/80 backdrop-blur-sm text-slate-800 border border-white/50 rounded-2xl rounded-bl-md'
                } p-4 shadow-lg`}>
                  <p className="text-sm leading-relaxed">{msg.isUser ? msg.message : msg.response}</p>
                  {!msg.isUser && (
            ...
            {chatLoading && (
              <div className="flex justify-start">
                <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl rounded-bl-md shadow-lg border border-white/50">
